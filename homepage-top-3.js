(function () {
    ...
    function init() {
        ...
        // ─── Mobil: posunout pod fixní/sticky hlavičku (kdyby překrývala) ─────
        // Geometrické měření: cokoliv ukotvené nahoře přes (skoro) celou šířku,
        // co zasahuje do horní části hero, se započítá jako překryv.
        function fixMobileOverlap() {
            ...
            for (var i = 0; i < all.length; i++) {
                ...
                if (r.top <= 2 && r.bottom > heroTop
                    && r.height > 0 && r.height < window.innerHeight * 0.6
                    && r.width > window.innerWidth * 0.5) {
                    cover = Math.max(cover, r.bottom - heroTop);
                }
            }
            if (cover > 0) hero.style.marginTop = Math.round(cover + 12) + 'px';
        }
        fixMobileOverlap();
        window.addEventListener('load', fixMobileOverlap);
        setTimeout(fixMobileOverlap, 400);
        setTimeout(fixMobileOverlap, 1000);
        var rsz;
        window.addEventListener('resize', function () { clearTimeout(rsz); rsz = setTimeout(fixMobileOverlap, 150); });
