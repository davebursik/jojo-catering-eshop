(function () {
    ...
    function init() {
        ...
        // ─── Mobil: posunout pod fixní hlavičku (kdyby překrývala) ────────────
        function fixMobileOverlap() {
            hero.style.marginTop = '';
            if (window.innerWidth > 767) return;
            var heroTop = hero.getBoundingClientRect().top;
            var fixedBottom = 0;
            var nodes = document.querySelectorAll('header,[class*="header"],[class*="navigation"],[class*="navbar"]');
            for (var i = 0; i < nodes.length; i++) {
                var cs = window.getComputedStyle(nodes[i]);
                if (cs.position === 'fixed') {
                    var r = nodes[i].getBoundingClientRect();
                    if (r.top <= 2 && r.height > 0 && r.height < window.innerHeight * 0.5) {
                        fixedBottom = Math.max(fixedBottom, r.bottom);
                    }
                }
            }
            if (fixedBottom > heroTop) hero.style.marginTop = Math.round(fixedBottom - heroTop + 12) + 'px';
        }
        fixMobileOverlap();
