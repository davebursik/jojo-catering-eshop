(function () {
    ...
    function anyShippingChecked() {
        var r = document.querySelectorAll('input[name="shippingId"]');
        for (var i = 0; i < r.length; i++) { if (r[i].checked) return true; }
        return false;
    }

    // state: 'active' | 'noaddress' | 'outofrange'
    // Idempotentní — bezpečné volat opakovaně z MutationObserveru (nezacyklí se).
    function setKuryrEnabled(state) {
        var wrapper = document.getElementById('shipping-29');
        if (!wrapper) return;

        var hintEl = document.getElementById('jojo-kuryr-hint');
        // Už aktivováno na tomto elementu? (active = bez hintu, šedý = s hintem)
        var applied = (wrapper.dataset.jojoState === state)
                   && (state === 'active' ? !hintEl : !!hintEl);
        if (applied) return;

        wrapper.dataset.jojoState = state;
        wrapper.style.display = 'block'; // překoná Shoptet skrytí
        ...
        // Šedý stav (noaddress / outofrange)
        wrapper.style.opacity = '0.45';
        wrapper.style.pointerEvents = 'none';

        // Osobní odběr jako výchozí — jen pokud nic platného není vybráno
        // nebo byl vybraný (teď zešedlý) kurýr.
        var kuryrRadio  = document.getElementById('shippingId-29');
        var pickupRadio = document.getElementById('shippingId-4');
        if (pickupRadio && (!anyShippingChecked() || (kuryrRadio && kuryrRadio.checked))) {
        ...
        hint.style.cssText = 'font-size:12px;padding:3px 4px 10px;line-height:1.5;';
        if (state === 'outofrange') {
            hint.style.color = '#c62828';
            hint.textContent = '❌ Vaše adresa je mimo dosah doručení (max. 100 km). Zvolte prosím osobní odběr.';
        } else {
            // noaddress — výchozí stav
            hint.style.color = '#888';
            hint.innerHTML = '📍 <a href="#" style="color:' + PRIMARY + ';text-decoration:underline;font-weight:600;">Zadejte adresu doručení</a> pro potvrzení ceny a aktivaci doručení.';
            var a = hint.querySelector('a');
    ...
    function init() {
        ...
        if (document.body.classList.contains('in-krok-1')) {
            ...
            // Stav kurýru podle uložené adresy
            var sess = loadSession();
            ...
            if (deliveryBox) {
                ...
                createWidget(krok1Inner, {
                    ...
                    onDelivery: function (canDeliver) {
                        ...
                        setKuryrEnabled(kuryrState);
                    },
                });
            }

            // Bez platné adresy vynutit Jojo region (do 15 km), aby Shoptet vyrenderoval
            // možnost „Jojo kurýr" (s cenou 150 Kč) — pak ji zešednem.
            if (kuryrState !== 'active') {
                switchRegion('Jojo rozvoz – do 15 km');
            }

            // Aplikovat stav teď i po (asynchronním) přerenderování seznamu dopravy.
            // setKuryrEnabled je idempotentní → observer se nezacyklí.
            setKuryrEnabled(kuryrState);
            var deliveryParent = deliveryBox
                ? (deliveryBox.parentNode || deliveryBox)
                : document.body;
            var kuryrObs = new MutationObserver(function () {
                setKuryrEnabled(kuryrState);
            });
            kuryrObs.observe(deliveryParent, { childList: true, subtree: true });
            setTimeout(function () { kuryrObs.disconnect(); }, 10000);
        }
