(function () {
    ...
    function init() {
        ...
        style.textContent = [
            ...
            '  .jojo-hero-wrap{margin-top:92px;margin-bottom:16px}',
        ...
        // ─── Mobil: dorovnat odsazení podle skutečné výšky fixní hlavičky ─────
        // CSS dává základ 92 px; tohle ho jen zpřesní, kdyby měl telefon vyšší
        // (nebo nižší) fixní hlavičku. Bezpečné — mění jen margin na mobilu.
        function refineMobileTop() {
            if (window.innerWidth > 767) { hero.style.marginTop = ''; return; }
            var h = 0;
            var els = document.querySelectorAll('body *');
            for (var i = 0; i < els.length; i++) {
                if (els[i] === hero || hero.contains(els[i])) continue;
                var cs = window.getComputedStyle(els[i]);
                if (cs.position !== 'fixed') continue;
                var r = els[i].getBoundingClientRect();
                if (r.top <= 1 && r.width >= window.innerWidth * 0.8
                    && r.height > 10 && r.height < window.innerHeight * 0.4) {
                    h = Math.max(h, r.bottom);
                }
            }
            if (h > 0) hero.style.marginTop = Math.round(h + 8) + 'px';
        }
        refineMobileTop();
        window.addEventListener('load', refineMobileTop);
        setTimeout(refineMobileTop, 600);
        var rsz;
        window.addEventListener('resize', function () { clearTimeout(rsz); rsz = setTimeout(refineMobileTop, 150); });
