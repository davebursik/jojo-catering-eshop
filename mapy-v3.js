(function () {
    ...
    function initKrok2() {
        ...
        // Debug: vypiš všechna pole formuláře do konzole (DevTools → Console)
        console.log('[JoJo] initKrok2 spuštěn. Session:', sess);
        var allInputs = document.querySelectorAll('input[name], select[name], textarea[name]');
        var fieldNames = [];
        allInputs.forEach(function (el) { fieldNames.push(el.name + ' (' + el.id + ')'); });
        console.log('[JoJo] Pole formuláře:', fieldNames.join(', '));

        // Předvyplnit doručovací adresu z kroku 1
        if (sess && sess.components) {
            var c = sess.components;
            fillField('#deliveryStreet',      c.street);
            fillField('#deliveryHouseNumber', c.houseNumber);
            fillField('#deliveryCity',        c.city);
            fillField('#deliveryZip',         c.zip);
            console.log('[JoJo] Předvyplněno:', c);
        }

        // Skrýt sekci s doručovací adresou (zákazník ji zadal v kroku 1)
        // Vyzkoušíme postupně různé selektory
        var deliverySectionSelectors = [
            '.co-delivery-address',
            '.co-delivery-info',
            '[data-testid="deliveryAddressSection"]',
            '.delivery-address',
        ];
        var hiddenDelivery = false;
        deliverySectionSelectors.forEach(function (sel) {
            var el = document.querySelector(sel);
            if (el && !hiddenDelivery) {
                el.style.display = 'none';
                hiddenDelivery = true;
                console.log('[JoJo] Skryta doručovací sekce:', sel);
            }
        });
        if (!hiddenDelivery) {
            console.warn('[JoJo] Doručovací sekce NENALEZENA — zkontroluj selektory v konzoli výše');
        }

        // Zobrazit banner s potvrzením adresy z kroku 1
        if (sess && sess.address) {
            ...
            banner.innerHTML = [
                ...
                '  <div>',
                '    <div style="font-size:13px;color:#555;font-weight:600;text-transform:uppercase;letter-spacing:.04em;">Doručovací adresa</div>',
            ...
            // Vložit banner na začátek hlavního formuláře / obsahu
            var insertTargetSelectors = [
                '.co-customer-information',
                '.co-step-content',
                '#order-step-2',
                'form.order-step-form',
                '.col-md-8',
            ];
            var inserted = false;
            insertTargetSelectors.forEach(function (sel) {
                var target = document.querySelector(sel);
                if (target && !inserted) {
                    target.insertAdjacentElement('afterbegin', banner);
                    inserted = true;
                    console.log('[JoJo] Banner vložen před:', sel);
                }
            });
            if (!inserted) {
                console.warn('[JoJo] Cílový selektor pro banner NENALEZEN — zkontroluj selektory');
            }
        }

        // Fakturační adresa: Shoptet ji ve výchozím stavu skrývá sám za checkboxem
        // Pokud ne, pomůžeme tomu:
        var billingToggleSelectors = [
            '#useBillingAddress',
            '#billingAddressDiffers',
            'input[name="useBillingAddress"]',
            'input[name="billingEqualsDelivery"]',
        ];
        billingToggleSelectors.forEach(function (sel) {
            var cb = document.querySelector(sel);
            if (cb) console.log('[JoJo] Nalezen billing checkbox:', sel, '| checked:', cb.checked);
        });
    }
