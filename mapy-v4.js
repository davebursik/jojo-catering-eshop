(function () {
    ...
    function createWidget(container, opts) {
        ...
        function showOk(km, zone) {
            ...
            result.innerHTML = [
                ...
                '    <div style="font-size:14px;color:#333;margin-top:3px;">Cena dopravy: <strong>' + zone.price + ' Kč</strong></div>',
                '  </div>',
                '</div>',
            ].join('');
        }
    ...
    function initKrok2() {
        ...
        // 1. Předvyplnit fakturační adresu ze session (shodná s doručovací)
        ...
        // Nastavit fakturační region
        if (zone) {
            var regionVal = setSelectByLabel('billRegionId', 'billRegionIdInput', zone.label);
        }

        // 2. Zaškrtnout "Doručit na jinou adresu" a předvyplnit doručovací pole
        var anotherCb = document.getElementById('another-shipping');
        if (anotherCb) {
            anotherCb.checked = true;
            // Shoptet's JS ukáže #shipping-address přes unveil — simulujeme kliknutí
            anotherCb.dispatchEvent(new Event('change', { bubbles: true }));
            anotherCb.dispatchEvent(new Event('click',  { bubbles: true }));
        }
        // Počkat až Shoptet odhalí sekci, pak vyplnit
        setTimeout(function () {
            fillField('#deliveryStreet',      comp.street);
            fillField('#deliveryHouseNumber', comp.houseNumber);
            fillField('#deliveryCity',        comp.city);
            fillField('#deliveryZip',         comp.zip);

            // Jméno pro doručení = kopírovat z fakturačního
            var billName = document.getElementById('billFullName');
            var delName  = document.getElementById('deliveryFullName');
            if (billName && delName) {
                delName.value = billName.value;
                billName.addEventListener('input', function () { delName.value = this.value; });
            }

            // Nastavit skrytý deliveryRegionId uvnitř #shipping-address
            if (zone) {
                var delRegionHidden = document.querySelector('#shipping-address input[name="deliveryRegionId"]');
                if (delRegionHidden) {
                    // Najdeme value ze select #billRegionId
                    var brs = document.getElementById('billRegionId');
                    if (brs) {
                        for (var i = 0; i < brs.options.length; i++) {
                            if (brs.options[i].text.trim() === zone.label) {
                                delRegionHidden.value = brs.options[i].value;
                                break;
                            }
                        }
                    }
        ...
        // 3. Skrýt "Doručit na jinou adresu" checkbox a doručovací formulář
        if (anotherCb) {
            var anotherRow = anotherCb.closest('.form-group');
            if (anotherRow) anotherRow.style.display = 'none';
        }
        // #shipping-address skryjeme po krátkém čekání (Shoptet ho nejdřív ukáže)
        setTimeout(function () {
            var shippingAddrSection = document.getElementById('shipping-address');
            if (shippingAddrSection) shippingAddrSection.style.display = 'none';
        }, 150);

        // 4. Banner s doručovací adresou nahoře
        if (sess && sess.address) {
            ...
            if (billingSection) {
                billingSection.insertAdjacentElement('beforebegin', banner);
            }
        }

        // 5. Toggle "Fakturační adresa je jiná než doručovací"
        var billingBox = document.querySelector('.co-billing-address');
        if (billingBox) {
            // Skrýt celou sekci fakturační adresy
            billingBox.style.display = 'none';

            // Přidat toggle před ni (banner ho vložil just before, takže toggle přidáme za banner)
            var toggleRow = document.createElement('div');
            toggleRow.style.cssText = 'margin-bottom:16px;';
            toggleRow.innerHTML = [
                '<label style="cursor:pointer;display:flex;align-items:center;gap:10px;font-size:15px;padding:12px 16px;border:1.5px solid #ddd;border-radius:8px;background:#fafafa;">',
                '  <input type="checkbox" id="jojo-billing-toggle" style="width:18px;height:18px;cursor:pointer;flex-shrink:0;" />',
                '  <span>Fakturační adresa je jiná než doručovací</span>',
                '</label>',
            ].join('');
            billingBox.insertAdjacentElement('beforebegin', toggleRow);

            document.getElementById('jojo-billing-toggle').addEventListener('change', function () {
                billingBox.style.display = this.checked ? '' : 'none';
            });
        }
    }
