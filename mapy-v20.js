(function () {
    ...
    // Shoptet vyrenderuje skutečnou možnost #shipping-29 jen po zadání adresy
    // (přepne dopravní region). Dokud adresa není zadaná, ukážeme vlastní vždy
    // viditelný box, aby zákazník věděl, že doručujeme — nezávisle na Shoptetu.
    ...
    // Vloží náš informační box nad seznam dopravy (jen jednou)
    function injectKuryrNotice(afterEl) {
        if (document.getElementById('jojo-kuryr-notice')) return;
        var n = document.createElement('div');
        n.id = 'jojo-kuryr-notice';
        n.style.cssText = 'border-radius:10px;padding:14px 16px;margin:0 0 14px;cursor:pointer;transition:background .15s,border-color .15s;';
        n.addEventListener('click', scrollToKrok1Widget);
        afterEl.insertAdjacentElement('afterend', n);
        showKuryrNotice('noaddress');
    }
    ...
    function showKuryrNotice(state) {
        ...
        if (state === 'outofrange') {
            ...
            n.innerHTML = [
                ...
                '  <div style="flex:1;">',
                '    <div style="font-weight:700;color:' + PRIMARY + ';font-size:14px;">Doručíme k vám — Jojo kurýr</div>',
                '    <div style="font-size:13px;color:#666;margin-top:2px;">Doručujeme až do 100 km! <strong>Zadejte adresu výše ☝</strong> a zobrazí se cena i možnost doručení.</div>',
                '  </div>',
    ...
    function activateKuryr() {
        ...
        function selectReal() {
            ...
            if (w) { w.style.display = 'block'; w.style.opacity = ''; w.style.pointerEvents = ''; }
            if (!radio.checked) { radio.checked = true; radio.dispatchEvent(new Event('change', { bubbles: true })); }
            return true;
        }
        if (selectReal()) return;
        // Shoptet teprve renderuje (AJAX po přepnutí regionu) — počkat
        var obs = new MutationObserver(function (m, o) { if (selectReal()) o.disconnect(); });
    ...
    function init() {
        ...
        if (document.body.classList.contains('in-krok-1')) {
            ...
            if (deliveryBox) {
                ...
                krok1Box.innerHTML = '<div style="font-size:16px;font-weight:700;margin-bottom:4px;">Doručujeme i k vám!</div><div style="font-size:13px;color:#777;margin-bottom:12px;">Zadejte adresu a aktivuje se doručení Jojo kurýrem.</div>';
                ...
                createWidget(krok1Inner, {
                    ...
                    onDelivery: function (canDeliver) {
                        if (canDeliver) activateKuryr();
                        else showKuryrNotice('outofrange');
                    },
                });

                // Vždy viditelný box „Jojo kurýr" nad seznamem dopravy
                injectKuryrNotice(krok1Box);
            }

            // Výchozí stav podle uložené adresy
            if (sess && sess.zone)            activateKuryr();
            else if (sess && sess.km !== undefined) showKuryrNotice('outofrange');
            else                              showKuryrNotice('noaddress');
        }
