(function () {
    ...
    function setKuryrEnabled(state) {
        ...
        // Vždy zobrazit — nikdy neskrýváme (display:block překoná i Shoptet CSS)
        ...
        if (state === 'active') {
            ...
            // Šedý + osobní odběr jako výchozí
            ...
            hint.style.cssText = 'font-size:12px;padding:3px 4px 8px;';
            if (state === 'outofrange') {
                hint.style.color = '#c62828';
                hint.textContent = '❌ Vaše adresa je mimo dosah doručení (max. 100 km).';
            } else {
                // noaddress — výchozí stav
                hint.style.color = '#888';
                hint.innerHTML = '📍 <a href="#" style="color:' + PRIMARY + ';text-decoration:underline;">Zadejte adresu doručení</a> pro potvrzení ceny.';
    ...
    function init() {
        ...
        if (document.body.classList.contains('in-krok-1')) {
            ...
            if (deliveryBox) {
                ...
                createWidget(krok1Inner, {
                    ...
                    onDelivery: function (canDeliver) {
                        setKuryrEnabled(canDeliver ? 'active' : 'outofrange');
                    },
                });
            }
            ...
            function applyKuryrState() {
                ...
                var sess = loadSession();
                // Pokud je v session platná zóna → active, jinak noaddress (šedý s výchozí cenou)
                setKuryrEnabled((sess && sess.zone) ? 'active' : 'noaddress');
                return true;
            }
            if (!applyKuryrState()) {
