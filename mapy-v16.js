(function () {
    ...
    // ─── Košík – widget pro zadání adresy ────────────────────────────────────

    function initKosikDelivery() {
        if (!document.body.classList.contains('in-kosik')) return;

        var cartTable = document.querySelector('.cart-content .cart-table');
        if (!cartTable) return;

        var section = document.createElement('div');
        section.id = 'jojo-kosik-delivery';
        section.style.cssText = 'background:#fff;border:1px solid #f0e8ed;border-radius:12px;padding:20px;margin:20px 0;box-shadow:0 2px 8px rgba(76,21,54,.06);';
        section.innerHTML = [
            '<div style="font-size:15px;font-weight:700;color:' + PRIMARY + ';margin-bottom:4px;">Chcete doručení Jojo kurýrem?</div>',
            '<div style="font-size:13px;color:#777;margin-bottom:14px;">Zadejte adresu a zjistíte cenu. Bez adresy se v dalším kroku zobrazí pouze osobní odběr.</div>',
        ].join('');
        var inner = document.createElement('div');
        section.appendChild(inner);
        cartTable.insertAdjacentElement('afterend', section);
        createWidget(inner, {});

        var jojoSkippedDelivery = false;

        var continueBtn = document.getElementById('continue-order-button');
        if (!continueBtn) return;

        continueBtn.addEventListener('click', function (e) {
            if (jojoSkippedDelivery) return;
            var sess = loadSession();
            if (sess && sess.address) return; // adresa zadána (v zóně i mimo)

            e.preventDefault();

            var overlay = document.getElementById('jojo-addr-overlay');
            if (overlay) { overlay.style.display = 'flex'; return; }

            overlay = document.createElement('div');
            overlay.id = 'jojo-addr-overlay';
            ...
            dialog.innerHTML = [
                ...
                '<div style="font-size:17px;font-weight:700;color:' + PRIMARY + ';margin-bottom:10px;text-align:center;">Nezadali jste adresu doručení</div>',
                '<div style="font-size:14px;color:#555;line-height:1.6;margin-bottom:22px;text-align:center;">',
                'Bez adresy se v dalším kroku nezobrazí možnost Jojo kurýrního doručení ',
                '– uvidíte <strong>pouze osobní odběr</strong>.',
                '</div>',
                '<button id="jojo-addr-enter" style="display:block;width:100%;padding:14px;background:' + PRIMARY + ';color:#fff;border:none;border-radius:10px;font-size:15px;font-weight:700;cursor:pointer;margin-bottom:10px;">',
                'Zadat adresu doručení',
                '</button>',
                '<button id="jojo-addr-skip" style="display:block;width:100%;padding:11px;background:transparent;color:#888;border:1.5px solid #ddd;border-radius:10px;font-size:14px;cursor:pointer;">',
                'Pokračovat bez doručení (osobní odběr)',
                '</button>',
            ].join('');
            ...
            document.getElementById('jojo-addr-enter').addEventListener('click', function () {
                overlay.style.display = 'none';
                var widget = document.getElementById('jojo-kosik-delivery');
                if (widget) widget.scrollIntoView({ behavior: 'smooth', block: 'center' });
            });

            document.getElementById('jojo-addr-skip').addEventListener('click', function () {
                jojoSkippedDelivery = true;
                overlay.remove();
                continueBtn.click();
            });
        });
    ...
    function init() {
        ...
        // Košík – widget pro zadání adresy doručení
        initKosikDelivery();
    }
