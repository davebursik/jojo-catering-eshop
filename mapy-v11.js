(function () {
    ...
    function createWidget(container, opts) {
        ...
        container.innerHTML = [
            ...
            '      ' + (opts.isHomepage ? 'Ověřit doručení' : 'Zjistit cenu'),
            '    </button>',
            '  </div>',
            '  <ul id="' + uid + '-sugg" style="list-style:none;padding:0;margin:0;border:1px solid #ddd;display:none;position:absolute;background:#fff;z-index:9999;width:100%;max-height:240px;overflow-y:auto;box-shadow:0 8px 24px rgba(0,0,0,.12);border-radius:8px;margin-top:4px;"></ul>',
    ...
    function init() {
        ...
        // Homepage – automatická injekce
        if (document.body.classList.contains('in-index')) {
            var hpAnchor = document.querySelector('.benefits-banners-line')
                        || document.querySelector('.hp-categories')
                        || document.querySelector('main#content');
            if (hpAnchor) {
                var hpWrapper = document.createElement('div');
                hpWrapper.style.cssText = 'padding:28px 0 12px;border-top:2px solid #f0e8ed;margin-top:4px;';
                hpWrapper.innerHTML = [
                    '<div style="font-size:21px;font-weight:800;color:' + PRIMARY + ';margin-bottom:4px;">Doručujeme i k vám?</div>',
                    '<div style="font-size:14px;color:#666;margin-bottom:14px;">Zadejte adresu a okamžitě zjistíte cenu dopravy.</div>',
                ].join('');
                var hpInner = document.createElement('div');
                hpWrapper.appendChild(hpInner);
                if (hpAnchor.classList.contains('benefits-banners-line')) {
                    hpAnchor.insertAdjacentElement('afterend', hpWrapper);
                } else {
                    hpAnchor.insertAdjacentElement('beforebegin', hpWrapper);
                }
